...
| `warmupSchedule` | Persisted reset-target schedule created by `teamclaude warmup reset` or `teamclaude warmup rolling`. Includes an IANA timezone; rolling mode also stores its absolute five-hour anchor. Prefer the CLI instead of editing this field by hand |
