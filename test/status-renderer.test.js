...
// The cap is the point: watch shows five clients, and ranking purely by
// lifetime tokens can push the machine that is actually running conversations
...
test('renderStatus ranks clients holding live sessions above idle heavier ones', () => {
  ...
  const output = renderStatus(status, { color: false, now, maxClients: 1 });

  assert.match(output, /\blight\b/);
  assert.doesNotMatch(output, /\bheavy\b/);
});
